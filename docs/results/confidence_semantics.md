...
> [!WARNING]
> **Strict Evaluation Rule**: No confidence value returned by SatQuery AI may be labeled as "accuracy", "ground-truth probability", "calibration", or "classification precision" in judge demonstrations, frontend views, or technical reports. Every confidence metric represents an uncalibrated algorithmic or pipeline artifact as detailed below.
...
4. **Presentation Prohibitions**: Presenters and pitch decks must never state:
   ...
   - *"Our confidence is calibrated"* (no empirical calibration experiments exist).
